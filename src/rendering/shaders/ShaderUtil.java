...
final class ShaderUtil {
    ...
    static Vec4fi calcSpecularAtFrag(int u, int v, int spec, int w, Material material) {
        ...
        if (material.hasSpecularMap()) {
            ...
            specColor = new Vec4fi(material.getDefualtSpecularColor(), Rasterfi.D_SHIFT_INV);//TODO: should store in mat
        }
    ...
    static Vec4fi sample_persp(int u, int v, BitmapABGR bitmapABGR, int z) {
        return bitmapABGR.getPixel_vInt(Rasterfi.toInt_inv(Rasterfi.multiply(u, z)), Rasterfi.toInt_inv(Rasterfi.multiply(v, z)),
                Rasterfi.D_SHIFT_INV);
    }


    static void sample_persp_NonAlloc(int u, int v, BitmapABGR bitmapABGR, int z, Vec4fi out) {
        bitmapABGR.getPixelNonAlloc_vInt_noshift(Rasterfi.multiply_noshift(u, z) >> 20,
                Rasterfi.multiply_noshift(v, z) >> 20,
                out);
    }
