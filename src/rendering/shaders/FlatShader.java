...
public class FlatShader implements IShader, IGeometryShader {
    ...
    public final VertexOut[] geom(VertexOut v1, VertexOut v2, VertexOut v3, Material material, VertexOut[] vertices) {
        ...
        //TODO: do not need to set_unsafe for all 3
    ...
    public static boolean fragNonAlloc(FlatInterpolants fI, Vec4fi surfaceColor, int spec,
                                       Vec4fi outColor, Vec4fi util, int y) {
        ...
        if (material.hasSpecularMap()) {
            ...
            util.mul_unsafe(spec, 2);
            outColor.add_unsafe(util);
        }
        if (material.hasTexture()) {
        ...
        Rasterfi.multiply_noshift(outColor, util);
        return true;
    }
