...
public class VertexOut {
    ...
    public VOutfi wDivideNew(Vec2f screenSpace_coord) {
        if (p_proj.w == 0)
            return new VOutfi(this);
        screenSpace_coord.x = moveToScreenSpace(p_proj.x * invW, RenderState.HALF_WIDTHf);
        screenSpace_coord.y = moveToScreenSpace(p_proj.y * invW, RenderState.HALF_HEIGHTf);

        Vec4fi vec = new Vec4fi(
                Rasterfi.floatToFixed_pos(screenSpace_coord.x),
                Rasterfi.floatToFixed_pos(screenSpace_coord.y),
                Rasterfi.floatToFixed(p_proj.z * invW),
                /*Rasterfi.floatToFixed_pos(p_proj.w)*/0,
                Rasterfi.D_SHIFT_POS
        );
    ...
    public VOutfi wDivideNew() {
        ...
        Vec4fi vec = new Vec4fi(
                Rasterfi.floatToFixed_pos(moveToScreenSpace(p_proj.x * invW, RenderState.HALF_WIDTHf)),
                Rasterfi.floatToFixed_pos(moveToScreenSpace(p_proj.y * invW, RenderState.HALF_HEIGHTf)),
                Rasterfi.floatToFixed(p_proj.z * invW),
                /*Rasterfi.floatToFixed_pos(p_proj.w)*/0,
                Rasterfi.D_SHIFT_POS
        );
