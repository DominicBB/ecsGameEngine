...
public abstract class Draw {
    ...
    public static void fillPolygon(VertexOut v1, VertexOut v2, VertexOut v3, RenderContext renderContext, Material material) {
        TriangleRasterizer.fillTriangle(v1, v2, v3, renderContext, material);
    }
    ...
    public static void wireframePolygon(Polygon p, RenderContext renderContext) {
        DrawLine3D.drawLine(p.vectors[0], p.vectors[1], white, white, renderContext);
        DrawLine3D.drawLine(p.vectors[1], p.vectors[2], white, white, renderContext);
        DrawLine3D.drawLine(p.vectors[2], p.vectors[0], white, white, renderContext);
    }
    ...
    public static void wireframePolygon(Triangle t, Vector3D c1, Vector3D c2, RenderContext renderContext) {
        DrawLine3D.drawLine(t.vectors[0], t.vectors[1], c1, c2, renderContext);
        DrawLine3D.drawLine(t.vectors[1], t.vectors[2], c1, c2, renderContext);
        DrawLine3D.drawLine(t.vectors[2], t.vectors[0], c1, c2, renderContext);
    }

    public static void drawLine(Vector3D v1, Vector3D v2, Vector3D c, Vector3D c2, RenderContext renderContext) {
        DrawLine3D.drawLine(v1, v2, c, c2, renderContext);
    }
